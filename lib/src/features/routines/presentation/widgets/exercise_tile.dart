...
class ExerciseTileState extends State<ExerciseTile>
  ...
  void _build(int sets) {
    ...
    for (var i = 0; i < _visibleSets; i++) {
      ...
      _rirCtl.add(TextEditingController(
          text: e?.rir.toString() ?? widget.detail.targetRir.toString()));
    }
  ...
  void _persist(int idx, {bool completed = false}) {
    widget.update(
      WorkoutLogEntry(
        ...
        rir: int.tryParse(_rirCtl[idx].text) ?? widget.detail.targetRir,
        completed: completed ||
  ...
  void logCurrentSet({required void Function(WorkoutLogEntry) addOrUpdate}) {
    ...
    addOrUpdate(
      WorkoutLogEntry(
        ...
        rir: int.tryParse(_rirCtl[current - 1].text) ?? widget.detail.targetRir,
        completed: true,
      ),
    );
  ...
  void _add() {
    ...
    setState(() {
      ...
      _rirCtl.add(TextEditingController(
          text: last?.rir.toString() ?? widget.detail.targetRir.toString()));
      _visibleSets++;
    });
    _persist(setNum - 1);
    widget.onChanged();
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Align(
                    ...
                    child: _info(
                      'Plan',
                      '${widget.detail.sets}x${widget.detail.reps} @${widget.detail.weight.toStringAsFixed(0)}kg • ${widget.detail.restSeconds}s • RIR ${widget.detail.targetRir}',
                    ),
                  ),
                ),
                if (widget.expanded) ...[
